import {increment, decrement, createStore} from "./store.js";

test("increment action should increment some value n", () => {
	const state = {n: 0};
	expect(increment(state).n).toBe(state.n + 1);

})
test("decrement action should decrement some value n", () => {
	const state = {n: 0};
